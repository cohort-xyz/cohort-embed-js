...
class CohortSDK {
   ...
   * @private
   */
  #renderIframe(options: IframeOptions, authConfig?: {
    userEmail: string;
      ...
      iframe.load(async () => {
        ...
        const unsubscribe = this.on('auth.updated', async payload => {
          ...
          if (!getAuthTokenCalled) {
            ...
            const authToken = await authConfig.getAuthToken();
  ...
  /**
   * Renders the Experience Space iframe.
  ...
  renderExperienceSpace(
    ...
    if (authConfig && !isEmail(authConfig.userEmail)) {
    ...
    const iframeOptions = {
      ...
      urlParams: {
        ...options.urlParams,
        embedEmail: authConfig ? authConfig.userEmail : undefined,
      },
    } satisfies IframeOptions;

    this.#renderIframe(iframeOptions, authConfig);
  }
